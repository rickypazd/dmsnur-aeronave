...
public class DependencyInjection {
  ...
  public static Object createInstance(Class _class, IMediator mediator)
    throws Exception {
    Constructor[] arr_constructores = _class.getConstructors();
    if (arr_constructores.length <= 0) {
      throw new Exception(
        "No se encontron los constructores para la clase: " + _class.getName()
      );
    }
    ...
    return constructor.newInstance(
      createParams(param_types, mediator).toArray()
    );
  }

  private static ArrayList<Object> createParams(
    Class[] param_types,
    IMediator mediator
  ) throws Exception {
